...
<title>secscan — {e(data.get('target',''))}</title>
<style>
...
_MS_LABEL = {"ok": "OK", "warn": "Åtgärda", "bad": "Brist", "info": "Info"}
...
def mailsec_html_report(info: dict) -> str:
    ...
    mx = ", ".join(info.get("mx", [])) or "ingen MX"
    ...
    rows = rows or '<tr><td colspan="3">inga svar</td></tr>'

    return f"""<!doctype html><html><head><meta charset="utf-8">
<title>secscan e-postsäkerhet — {e(info.get('domain',''))}</title>
<style>
 body{{font:14px/1.5 system-ui,sans-serif;margin:2rem;color:#1a1a1a;background:#fafafa}}
...
<h1>E-postsäkerhet — {e(info.get('domain',''))}</h1>
<div><span class="grade">{e(str(grade))}</span><span class="score"><strong>{info.get('score',0)}</strong>/100</span></div>
<div class="meta">Mailserver: {e(mx)}{e(provider)}</div>
<table><tr><th>Status</th><th>Kontroll</th><th>Detalj &amp; åtgärd</th></tr>{rows}</table>
<p class="meta">Genererad av secscan {e(_version())}. Endast för auktoriserad testning.</p>
</body></html>"""
