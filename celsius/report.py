...
<div class="meta">{len(scans)} host(s) across {len(unique_ips)} unique IP(s) &middot; aggregated from the latest stored scan per host</div>
<div style="margin:.5rem 0 1.5rem">{chips}</div>
<h2>Hosts</h2>
<table><tr><th>Host</th><th>IP</th><th>Worst</th><th>CVEs</th><th>Findings</th><th>Services</th><th>Scanned</th></tr>{overview}</table>
{sections}
<p class="meta">Generated by celsius {e(_version())}. For authorized testing only.</p>
</body></html>"""
